...
static struct list_node* find_document (int doc_id)
{
...
static int len_list (struct list_node* ln)
{
	int count = 0;
	while (ln)
	{
...
static struct list_node* create_list_node (int               doc_id,
                                           char*             name,
                                           int               word_count,
...
int listdb_add (char* name, int word_count, char* author)
{
	...
	tmp = create_list_node (g_this_id_2, name, word_count, g_linked_list);
	if (!tmp)
	{
		return -1;
	}
...
void listdb_stat (void)
{
	...
	printf ("List size matches expected? -> %c\n",
	        ((g_this_id_2 == len_list (g_linked_list)) ? 'Y' : 'N'));
}
